...
assigned: 2025-04-25 09:00:00.00-8
...
using only public methods of the bst class (insert, remove, contains, count, printDeck, begin, end, rbegin, rend) and the Iterator class.
...
1. BST Public Methods: Include at least 5 test cases for each public member function of the bst class, including at a minimum insert(), remove(), contains(), count(), printDeck(), convert(), getSuccessorNode(), and getPredecessorNode(). Test cases should cover:
* Empty tree (e.g., verify contains() returns false, count() returns 0).
...
For this lab, you will have a lot of flexibility in your implementation, but your mentor will check for the following requirements when reviewing your code. Ensure your solution meets these criteria:

- You must implement a custom binary search tree in `bst.h` and `bst.cpp` to solve the problem, not another data structure or a class from the standard library. You must provide two implementations: one using STL `std::set` (a balanced BST) in `main_set.cpp` and another using your custom BST in `main.cpp`.
- Your `bst` class must implement a constructor, a destructor, and all necessary methods to support the game, including at least `insert(const Card&)`, `remove(int)`, `contains(int)`, `count()`, `printDeck()`, `convert(const Card&)`, `getSuccessorNode(int)`, and `getPredecessorNode(int)`.
...
- Implement a free function `playGame(bst& alice, bst& bob)` in `bst.cpp` to manage the game logic, using only public `bst` methods (`insert`, `remove`, `contains`, `count`, `printDeck`, `begin`, `end`, `rbegin`, `rend`) and the `Iterator` class. Do not access `Node` pointers directly; use the iterator’s `operator->` for card data (e.g., `it->card.toString()`, `it->info`) and public methods for all operations.
- Use the `Card` class (defined in `card.h` or `bst.h`) to represent cards, with `insert(const Card&)` accepting `Card` objects instead of strings.
